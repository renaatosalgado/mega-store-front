...
export default function EletroDept() {
    ...
    async function handleEletro() {
        try {
            ...
            console.log(eletroPromise.data);
            ...
            <Container>
                ...
                <SubContainer>
                    <ItemsContainer isLoading={isLoading}>
                        {eletros.length === 0 && isLoading && <span><ThreeDots color='#0377FF' height={25} width={150} /></span>}
                        {eletros.length !== 0 && !isLoading &&
                            Array.from(eletros).map((eletro, id) =>
                                <Items onClick={() => navigate(`/product/${eletro._id}`)} key={id}>
                                    <div className="image-wrapper">
                                        <img className="product-image" src={eletro.image} alt="ps" />
                                    </div>
                                    <div className="info-wrapper">
                                        <span className="productName">{eletro.name}</span>
                                        <span className="productPrice">R$ {eletro.price.toFixed(2).replace('.', ',')}</span>
                                    </div>
                                </Items>
                            )
                        }
