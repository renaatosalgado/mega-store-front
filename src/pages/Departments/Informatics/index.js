...
export default function InformaticsDept() {
    ...
    async function handleInformatics() {
        try {
            ...
            console.log(informaticsPromise.data);
            ...
            <Container>
                ...
                <SubContainer>
                    <ItemsContainer isLoading={isLoading}>
                        {informatics.length === 0 && isLoading && <span><ThreeDots color='#0377FF' height={25} width={150} /></span>}
                        {informatics.length !== 0 && !isLoading &&
                            Array.from(informatics).map((informatic, id) =>
                                <Items onClick={() => navigate(`/product/${informatic._id}`)} key={id}>
                                    <div className="image-wrapper">
                                        <img className="product-image" src={informatic.image} alt="ps" />
                                    </div>
                                    <div className="info-wrapper">
                                        <span className="productName">{informatic.name}</span>
                                        <span className="productPrice">R$ {informatic.price.toFixed(2).replace('.', ',')}</span>
                                    </div>
                                </Items>
                            )
                        }
