...
export default function FashionDept() {
    ...
    async function handleFashion() {
        try {
            ...
            console.log(fashionPromise.data);
            ...
            <Container>
                ...
                <SubContainer>
                    <ItemsContainer isLoading={isLoading}>
                        {fashions.length === 0 && isLoading && <span><ThreeDots color='#0377FF' height={25} width={150} /></span>}
                        {fashions.length !== 0 && !isLoading &&
                            Array.from(fashions).map((fashion, id) =>
                                <Items onClick={() => navigate(`/product/${fashion._id}`)} key={id}>
                                    <div className="image-wrapper">
                                        <img className="product-image" src={fashion.image} alt="ps" />
                                    </div>
                                    <div className="info-wrapper">
                                        <span className="productName">{fashion.name}</span>
                                        <span className="productPrice">R$ {fashion.price.toFixed(2).replace('.', ',')}</span>
                                    </div>
                                </Items>
                            )
                        }
